class Array:
        ...
        def __init__(self, parm1, initialvalue=None):
                ...
                self.innerlist = []                  # the actual list that stores our items
                self.count_items = 0           # how many elements are in it right now
                self.the_type = None          # a string that tells what the types are
                if type(parm1) == list:
                        self.size = len(parm1)
                        for x in parm1:
                                self.innerlist.append(x)
                        self.count_items = len(parm1)
                        self.the_type = type(parm1[0]).__name__
                else:
                        self.size = parm1
